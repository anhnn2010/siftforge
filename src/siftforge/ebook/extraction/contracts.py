...
EBOOK_PAGE_PROMPT_V5_R2 = PromptSpec(
    name="ebook_page_evidence",
    version="5.2",
    text="""Extract faithful page-local evidence from this scanned book page.
...
- A recurring label such as "TÌNH HUỐNG" is scenario_label even on a page where no
  separate scenario title follows. Use scenario_title for the specific scenario name,
  such as "Bật lại!", when one is visibly present.
- Do not merge adjacent standalone title/subtitle/label lines into one block merely
  because they are centered, visually similar, or close together. If each line is a
  distinct phrase with its own semantic identity, emit a separate block.
- A short standalone supporting line immediately below a major title may use
  heading_role_hint=subtitle when that local role is clear. Keep neighboring subtitle
  lines as separate blocks rather than concatenating them.
- If two adjacent title-like lines each carry their own superscript footnote marker,
  they MUST be separate blocks. Preserve each footnote marker as its own superscript
  span; never flatten the marker into baseline heading text.
- Use list_item for one locally apparent enumeration/list item, not for a whole list.
- Use verse for poetry, lyrics, ca dao, or other line-oriented verse whose semantic
  line boundaries matter.
- Use quote for prose quotation content. Multiple quoted paragraphs may be separate
...
- Use unknown rather than guessing when local glyph evidence is insufficient, and add a
  concise warning when practical.
- Use multiple spans whenever source typography or span language changes.
- A visible footnote-reference digit/letter raised above the baseline must be emitted
  as a separate span with vertical_position=superscript, including inside headings,
  subtitles, labels, captions, and other short title-like text.
...
EBOOK_PAGE_PROMPT = EBOOK_PAGE_PROMPT_V5_R2
